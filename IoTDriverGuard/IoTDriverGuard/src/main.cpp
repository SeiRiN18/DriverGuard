...
confidence = constrain(confidence, 0.0f, 1.0f);
  ...
  const bool eventTypeChanged =
    (lastReportedEventType == nullptr) ||
    (std::strcmp(eventType, lastReportedEventType) != 0);
  const bool severityChanged = (severity != lastReportedSeverity);

  if ((eventTypeChanged || severityChanged) &&
      (millis() - lastSend >= SEND_INTERVAL_MS)) {
    lastSend = millis();
    ...
    if (!sendEventToServer(ev)) {
      bufferEvent(eventType, severity, confidence);
    } else {
...
if (WiFi.isConnected() && hasBufferedEvents()) {
  DriverEvent ev = getNextBufferedEvent();
  if (sendEventToServer(ev)) {
    popBufferedEvent();
  }
}
