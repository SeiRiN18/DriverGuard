...
void updateDisplay(float drowsy, float attention, int severity, bool headDown) {
  ...
  display.setCursor(0, 0);  display.printf("Drowsy: %d%%",    (int)(drowsy    * 100));
  display.setCursor(0, 12); display.printf("Attn:   %d%%",    (int)(attention * 100));
  display.setCursor(0, 24); display.print("Head: "); display.print(headDown ? "DOWN" : "OK");
  display.setCursor(0, 40); display.print("Risk: ");
  for (int i = 1; i <= 5; i++)
    display.print(i <= severity ? (char)219 : (char)176);
  display.display();
}

void setup() {
  Serial.begin(115200);
  Serial.println("[Boot] DriverGuard IoT starting...");
...
void loop() {
  ...
  bool  headDown = false;
  if (mpuOk && readAccel(ax_g, ay_g, az_g))
    headDown = (abs(ax_g) > 0.4f || abs(ay_g) > 0.4f);
  ...
  int severity = (score <= 1) ? 1 : (score == 2) ? 2 : (score == 3) ? 3 : (score == 4) ? 4 : 5;

  float confidence = constrain(
    drowsy * 0.4f + (1.0f - attention) * 0.4f + (headDown ? 0.2f : 0.0f),
    0.0f, 1.0f
  );

  if      (severity < 5) { setLedColor(0, 1, 0); }
  ...
  if (severity > 2) {
    eventType = (drowsy >= (1.0f - attention)) ? "DROWSINESS" : "ATTENTION_LOSS";
  }

  const bool typeChanged     = (lastReportedEventType == nullptr) || (strcmp(eventType, lastReportedEventType) != 0);
  const bool severityChanged = (severity != lastReportedSeverity);
  const unsigned long elapsed = millis() - lastSend;

  const bool sendOnChange    = (typeChanged || severityChanged) && (elapsed >= SEND_INTERVAL_MS);
  const bool sendHeartbeat   = (elapsed >= HEARTBEAT_INTERVAL_MS);

  if (sendOnChange || sendHeartbeat) {
    Serial.printf("[Loop] Sending: type=%s sev=%d conf=%.2f reason=%s\n",
      eventType, severity, confidence, sendHeartbeat && !sendOnChange ? "heartbeat" : "change");

    DriverEvent ev = { eventType, severity, confidence, millis() };

    if (sendEventToServer(ev)) {
      lastReportedSeverity  = severity;
      lastReportedEventType = eventType;
      lastSend = millis();
      Serial.println("[Loop] Send OK");
    } else {
      Serial.println("[Loop] Send FAILED, will retry next cycle");
    }
  }
